"""
Test script for the AI processor.
"""
import os
import sys
import argparse
...
from src.utils.config import PROCESSED_DIR, AI_OUTPUT_DIR, LOG_LEVEL, LOG_FILE
...
def test_ai_processor(input_path, api_key, base_url=None):
    """
    Test the AI processor with a sample Excel file.
    ...
    """
    try:
        logger.info(f"Testing AI processor with {input_path}")
        ...
        output_path = os.path.join(output_dir, f"test_filtered_{basename}")
        ...
        logger.info(f"Full filtered data saved to: {os.path.join(output_dir, f'full_filtered_{basename}')}")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Test the AI processor')
    ...
    # Run the test
    success = test_ai_processor(input_path, api_key, args.base_url)
