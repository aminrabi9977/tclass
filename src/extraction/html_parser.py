...
class TenderHTMLParser:
    ...
    def __init__(self):
        """Initialize the HTML parser."""
        # Define the fields to extract (updated with new fields and renamed columns)
        self.fields = [
            "شماره مناقصه در هزاره",   # Tender number in Hezareh
            "عنوان",                   # Title
            "برگزارکننده",              # Organizer
            "منطقه",                   # Region
            "تاریخ انتشار",             # Publication date
            "تهیه اسناد تا تاریخ",       # Document preparation deadline (renamed)
            "منبع",                    # Source
            "ارسال اسناد تا تاریخ",      # Document submission deadline (renamed)
            "شرح آگهی",                # Description
            "شرایط آگهی",               # Conditions
            "آدرس",                    # Address (NEW)
            "وبسایت",                  # Website (NEW)
            ...
            for row in rows:
                ...
                for cell in cells:
                    ...
                    if "تاریخ انتشار" in label:
                        ...
                        tender_data["تهیه اسناد تا تاریخ"] = self._extract_specific_field_value(cell)
                    elif "منبع" in label:
                        tender_data["منبع"] = self._extract_specific_field_value(cell)
                    elif "ارسال اسناد تا" in label:
                        tender_data["ارسال اسناد تا تاریخ"] = self._extract_specific_field_value(cell)
                    elif "شرح آگهی" in label:
                        description_span = cell.find('span', class_='cd-noticedescription')
                        if description_span:
                            tender_data["شرح آگهی"] = description_span.text.strip()
                        ...
                        # NEW: Extract address
                        tender_data["آدرس"] = self._extract_specific_field_value(cell)
                    elif "وبسایت" in label:
                        website_link = cell.find('a')
                        if website_link and website_link.get('href'):
                            href = website_link['href'].strip()
                            
                            # شرایط استثناء
                            if href == "https://www.setadiran.ir":
                                pass  # هیچ مقداری ذخیره نشود
                            elif "eproc.setadiran.ir" in href:
                                pass
                            elif href.startswith("https://www.hezarehinfo.net/notice/emailimg"):
                                pass
                            else:
                                tender_data["وبسایت"] = href
                        else:
                            tender_data["وبسایت"] = self._extract_specific_field_value(cell)
                    elif "دسته بندی" in label or "کلیدواژه" in label:
                        tender_data["دسته بندی"] = self._extract_specific_field_value(cell)
